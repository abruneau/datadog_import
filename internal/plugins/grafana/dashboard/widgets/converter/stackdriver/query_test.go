...
func TestFunction(t *testing.T) {
	...
	assert.Equal(t, datadog.FORMULAANDFUNCTIONMETRICFUNCTION_RATE, f)
	testTarget = Query{&Target{}, false}
	testTarget.MetricQuery.PerSeriesAligner = "ALIGN_DELTA"
	f = testTarget.function()
	assert.Equal(t, datadog.FORMULAANDFUNCTIONMETRICFUNCTION_COUNT, f)
	testTarget = Query{&Target{}, false}
	...
	f = testTarget.function()
	assert.Equal(t, datadog.FormulaAndFunctionMetricFunction(""), f)
}
